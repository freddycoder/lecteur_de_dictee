...
if __name__ == "__main__" :
    ...
    if os.path.exists("utl/parametres_lecture.val"):
        try:
            ...
            print("Une erreur est survenu lors du chargement des paramètres. Le contenu du fichier n'est pas valide. Les paramètres par défaut vont être chargés")
            parametre = ParametresLecture()

    # Chager la dernière dictée non enregistrer
    dictee = Dictee()
    if os.path.exists("utl/dictee_non_enregistrer.dct"):
        try:
    ...
    dictee.sauvegarder("utl/dictee_non_enregistrer.dct")